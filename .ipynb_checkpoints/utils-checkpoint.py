...
def generate_mock_data(num_customers: int=20) -> pd.DataFrame:
    ...
    #
    # Pivot date =  a spine column
    #
    pivot_dates = [datetime.now().strftime("%Y-%m-%d") for _ in range(num_customers)]
    #
    # unique customer id
    #
    ...
    #
    approved_dates = [f"2024-{random.randint(1, 12)}-{random.randint(1, 31)}" for _ in range(num_customers)]
    #
    # days delinquent
    #
    probs = [1 if random.random() > 0.75 else 0 for _ in range(num_customers)]
    days_delinquent = [random.randint(0, 365) for _ in range(num_customers)]
...
def eval_data(data: pd.DataFrame, threshold: int = 3) -> pd.DataFrame:
    ...
    #
    data['months_delinquent'] = data['days_delinquent'] // 30 # 30 provided by Kamal
    #
    # calculate cost to fragile
    #
    data['cost_to_fragile'] = data['months_delinquent'] * data['monthly_recurr_revenue']
    #
    # measure impact
    #
    def _measure_impact(months):
        if months == 0:
            return "green"
        elif months <= threshold:
            return "yellow"
        else:
            return "red"
            
    data['thermostat'] = data.apply(lambda row: _measure_impact(row['months_delinquent']), axis=1)
...
def generate_pie_chart(data: pd.DataFrame) -> plt.Figure:
    ...
    # define labels
    #
    y_mon = round(data[data['thermostat'] == 'yellow']['cost_to_fragile'].sum().item(), 2)
